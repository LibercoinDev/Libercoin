    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Libercoin and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002Libercoin per transaction on average.
            */
            if(r <= 20)
            {
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeLibercoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fLibercoinSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting darksend for %f Libercoin CENT - sessionTotalValue %d\n", fLibercoinSubmitted, sessionTotalValue);
...
void CDarkSendPool::GetDenominationsToString(int nDenom, std::string& strDenom){
    ...
    //
    // bit 0 - 100Libercoin+1 ( bit on if present )
    // bit 1 - 10Libercoin+1
    // bit 2 - 1Libercoin+1
    // bit 3 - .1Libercoin+1
    // bit 3 - non-denom
...
int CDarkSendPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget){
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations){
        if(nDenomTarget != 0){
            ...
            else if((nDenomTarget & (1 << 6)) && v == ((.01*COIN)   +10)) {fAccepted = true;}
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("Libercoin-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeLibercoinAmount = (rand() % nLeftToAnon)+3;
                } else {

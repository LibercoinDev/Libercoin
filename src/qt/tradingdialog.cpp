    ...
    model(0)
{
    ...
    ui->LibercoinAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->CryptopiaLibercoinLabel->setTextFormat(Qt::RichText);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "SUM(BTC)" << "TOTAL(BTC)" << "Libercoin(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)" << "Libercoin(SIZE)" << "TOTAL(BTC)" << "SUM(BTC)");
    /*OrderBook Table Init*/

    /*Market History Table Init*/
    ui->MarketHistoryTable->setColumnCount(5);
    ui->MarketHistoryTable->verticalHeader()->setVisible(false);
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(Libercoin)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //Libercoinst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://cryptopia.co.nz/api/GetMarket/Libercoin_BTC");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://cryptopia.co.nz/api/GetMarketOrders/Libercoin_BTC");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://cryptopia.co.nz/api/GetMarketHistory/Libercoin_BTC");
      return Response;
}
...
QString tradingDialog::BuyLibercoin(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://cryptopia.co.nz";
            ...
            URL += "/api/SubmitTrade";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-Libercoin&quantity=";
...
QString tradingDialog::SellLibercoin(QString OrderType, double Quantity, double Rate){

    QString str = "";
    QString URL = "https://cryptopia.co.nz";
            URL += OrderType;
            URL += "/api/GetMarket/Libercoin_BTC";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-Libercoin&quantity=";
            URL += str.number(Quantity,'i',8);
            URL += "&rate=";
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://cryptopia.co.nz/api/GetOpenOrders";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-Libercoin";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://cryptopia.co.nz/api/GetDepositAddress";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=Libercoin";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://cryptopia.co.nz/api/GetTradeHistory";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-Libercoin&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    QJLibercoinbject obj = GetResultObjectFromJSONArray(Response);
    ...
    ui->volumet->setText("<b>Libercoin Volume:</b> <span style='font-weight:bold; font-size:12px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> Libercoin");
...
void tradingDialog::DisplayBalance(QLabel &BalanceLabel,QLabel &Available, QLabel &Pending, QString Currency,QString Response){
    ...
    //Set the labels, parse the json result to get values.
    QJLibercoinbject ResultObject = GetResultObjectFromJLibercoinbject(Response);
...
void tradingDialog::DisplayBalance(QLabel &BalanceLabel, QLabel &BalanceLabel2, QString Response, QString Response2){
    ...
    //Set the labels, parse the json result to get values.
    QJLibercoinbject ResultObject = GetResultObjectFromJLibercoinbject(Response);
    QJLibercoinbject ResultObject2 = GetResultObjectFromJLibercoinbject(Response2);
...
void tradingDialog::ParseAndPopulateOpenOrdersTable(QString Response){
    ...
    QJsonArray jsonArray = GetResultArrayFromJLibercoinbject(Response);
    QJLibercoinbject obj;
...
void tradingDialog::CancelOrderSlot(int row, int col){
   ...
   if (reply == QMessageBox::Yes) {
          ...
          QJLibercoinbject ResponseObject = jsonResponse.object();
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    QString str;
    QJLibercoinbject obj;
    QJLibercoinbject ResultObject = GetResultObjectFromJLibercoinbject(OrderBook);
    ...
    double LibercoinSupply = 0;
    double LibercoinDemand = 0;
    double BtcSupply = 0;
    double BtcDemand = 0;
    ...
    ui->LibercoinSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(LibercoinSupply,'i',8) + "</span><b> Libercoin</b>");
    ...
    ui->LibercoinDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(LibercoinDemand,'i',8) + "</span><b> Libercoin</b>");
...
void tradingDialog::ActionsOnSwitch(int index = -1){
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    Response2 = GetBalance("Libercoin");
                    Response3 = GetOrderBook();

                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->LibercoinAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    Response = GetBalance("Libercoin");
                    Response2 = GetBalance("BTC");
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->CryptopiaLibercoinLabel, *ui->CryptopiaBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("Libercoin");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->LibercoinBalanceLabel,*ui->LibercoinAvailableLabel_2,*ui->LibercoinPendingLabel, QString::fromUtf8("Libercoin"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputLibercoin->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    QString balance = GetBalance("Libercoin");
    QString buyorders = GetOrderBook();

    QJLibercoinbject BuyObject = GetResultObjectFromJLibercoinbject(buyorders);
    QJLibercoinbject BalanceObject =  GetResultObjectFromJLibercoinbject(balance);
    QJLibercoinbject obj;

    double AvailableLibercoin = BalanceObject["Available"].toDouble();
...
void tradingDialog::on_UpdateKeys_clicked(bool Save, bool Load)
{
  ...
  this->SecretKey = ui->SecretKeyInput->text();
   ...
   QJLibercoinbject ResponseObject = jsonResponse.object();                                 //get json obj
...
{
    //calculate amount of BTC that can be gained from selling Libercoin available balance
    QString responseA = GetBalance("Libercoin");
    QString str;
    QJLibercoinbject ResultObject =  GetResultObjectFromJLibercoinbject(responseA);

    double AvailableLibercoin = ResultObject["Available"].toDouble();

    ui->UnitsInputLibercoin->setText(str.number(AvailableLibercoin,'i',8));
}
...
void tradingDialog::on_Buy_Max_Amount_clicked()
{
    ...
    QJLibercoinbject ResultObject =  GetResultObjectFromJLibercoinbject(responseA);
    QJLibercoinbject ResultObj    =  GetResultObjectFromJSONArray(responseB);
...
{
    double Quantity = ui->CryptopiaLibercoinLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableLibercoin,'i',8));
}

QJLibercoinbject tradingDialog::GetResultObjectFromJLibercoinbject(QString response){

    QJsonDocument jsonResponse = QJsonDocument::fromJson(response.toUtf8());          //get json from str.
    QJLibercoinbject  ResponseObject = jsonResponse.object();                              //get json obj
    QJLibercoinbject  ResultObject   = ResponseObject.value(QString("result")).toObject(); //get result object

  return ResultObject;
}

QJLibercoinbject tradingDialog::GetResultObjectFromJSONArray(QString response){

    QJsonDocument jsonResponsea = QJsonDocument::fromJson(response.toUtf8());
    QJLibercoinbject   jLibercoinbjecta   = jsonResponsea.object();
    QJsonArray    jsonArraya    = jLibercoinbjecta["result"].toArray();
    QJLibercoinbject   obj;
...
QJsonArray tradingDialog::GetResultArrayFromJLibercoinbject(QString response){

    QJsonDocument jsonResponse = QJsonDocument::fromJson(response.toUtf8());
    QJLibercoinbject   jLibercoinbject   = jsonResponse.object();
    QJsonArray    jsonArray    = jLibercoinbject["result"].toArray();
...
{
    QString response = GetMarketSummary();
    QJLibercoinbject ResultObject = GetResultObjectFromJSONArray(response);
    QString Str;
...
void tradingDialog::on_BuyLibercoin_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyLibercoin(Order,Quantity,Rate);
...
void tradingDialog::on_SellLibercoinBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputLibercoin->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputLibercoin->text();
            Msg += " Libercoin @ ";
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellLibercoin(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJLibercoinbject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellLibercoin(Order,y,x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJLibercoinbject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                QString SellResponse = SellLibercoin(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJLibercoinbject SellResponseObject = SelljsonResponse.object();                              //get json obj
                ...
                if (SellResponseObject["success"].toBool() == false){
                    ...
                    if (ResponseObject["success"].toBool() == false){
                        ...
                        if (ResponseObject["success"].toBool() == false){
                            ...
                            QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" Libercoin for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                        }
                    } else if (ResponseObject["success"].toBool() == true){
                        QMessageBox::information(this,"Success","<center>Cross-Send Successful</center>\n Sold "+Astr.number(Qty,'i',4)+" Libercoin for "+Qstr.number((ui->CSUnitsInput->text().toDouble()-0.0002),'i',8)+" BTC");
                    }
...
void tradingDialog::on_UnitsInputLibercoin_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}

...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Libercoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Libercoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeLibercoinamount=<n> " + _("Keep N Libercoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Libercoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Libercoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Libercoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Libercoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Libercoin to complete") << "\n";
    ...
    nAnonymizeLibercoinAmount = GetArg("-anonymizeLibercoinamount", 0);
    if(nAnonymizeLibercoinAmount > 999999) nAnonymizeLibercoinAmount = 999999;
    if(nAnonymizeLibercoinAmount < 2) nAnonymizeLibercoinAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeLibercoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize Libercoin Amount %d\n", nAnonymizeLibercoinAmount);

    /* Denominations
       A note about convertability. Within luxsend pools, each denomination
       is convertable to another.
       For example:
       1Libercoin+1000 == (.1Libercoin+100)*10
       10Libercoin+10000 == (1Libercoin+1000)*10
    */

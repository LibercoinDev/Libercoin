...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLibercoinAmount"))
        settings.setValue("nAnonymizeLibercoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLibercoinAmount = settings.value("nAnonymizeLibercoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLibercoinAmount"))
        SoftSetArg("-anonymizeLibercoinamount", settings.value("nAnonymizeLibercoinAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case luxsendRounds:
            ...
            break;
        case anonymizeLibercoinAmount:
            nAnonymizeLibercoinAmount = value.toInt();
            settings.setValue("nAnonymizeLibercoinAmount", nAnonymizeLibercoinAmount);
            emit anonymizeLibercoinAmountChanged(nAnonymizeLibercoinAmount);
            break;
        default:
            break;
        }

...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Libercoin"))
        return false;
...
{
    // Convert Libercoin:// to Libercoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Libercoin://"))
    {
        uri.replace(0, 12, "Libercoin:");
    }
...
boost::filesystem::path static GetAutostartDir()
{
    ...
    char* pszConfigHome = getenv("Libercoin_CONFIG_HOME");
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Libercoin-Qt") + " " + tr("version") + " " +
    ...
    setWindowTitle(tr("Libercoin-Qt"));

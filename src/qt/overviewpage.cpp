    ...
    filter(0)
{
 ...
 // start with displaying the "out of sync" warnings
    ...
    if(fMasterNode || fLiteMode){
        ...
        ui->toggleluxsend->setText(tr("Start hiddensend"));
    } else {
        ui->toggleluxsend->setText(tr("Stop hiddensend"));
    }
...
void OverviewPage::updateluxsendProgress()
{
    ...
    int64_t nMaxToAnonymize = nAnonymizeLibercoinAmount*COIN;
...
void OverviewPage::darkSendStatus()
{
    ...
    if(nBestHeight != darkSendPool.cachedNumBlocks)
    {
        ...
        strSettings.prepend(BitcoinUnits::formatWithUnit(
            ...
            nAnonymizeLibercoinAmount * COIN)
        );
...
void OverviewPage::toggleluxsend(){
    ...
    if(!fEnableluxsend){
        ...
        if(nAnonymizeLibercoinAmount == 0){
